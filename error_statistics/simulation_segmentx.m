% function [std_dev2,numberfail,numberkV,numberrep,meandis] = simulation_segmentx(j0,so3d,conditions)
% function [std_dev2,aberror2,ab_dev2,parameters,threed,twod] = simulation_segmentx(j0,so3d,conditions)
function [std_dev2,aberror2,ab_dev2,numberfail,numberkV,perct95_3d,perct95_2d,perct99_3d,perct99_2d] = simulation_segmentx(j0,so3d,conditions)
...
threshold = conditions(26);
iskvon = conditions(27);
...
norm3de = zeros(points_period,1);
normdev = zeros(points_period,1);
...
while jj <= points_period
    ...
    if jj<=runlength
        ...
        if jj == 1
            checkpoint(jj,:) = r3d(jj,:);
            checkpoint(jj+1,:) = checkpoint(jj,:);
        elseif jj < runlength+1 % when position is measured, instead of estimated.
            deviat(jj,:) = r3d(jj,:) - checkpoint(jj,:);
            displs(jj,:) = so3d(jj,:) - checkpoint(jj,:);
%             if norm(deviat(jj,:)) > threshold
%                 checkpoint(jj+n_lag,:) = r3d(jj,:);
%                 isrep(jj+n_lag) = true;
%             else
%                 checkpoint(jj+n_lag,:) = checkpoint(jj,:);
                  checkpoint(jj+n_lag,:) = r3d(jj,:);
%             end
        end
...
%         if iskV(jj)  && norm(r3d(jj,:)-checkpoint(jj,:)) > threshold
%             isrep(jj) = true; % to decide to reposition on point jj+n_lag 
%         end
