function plotfiguresx(i0,conditionx,modex,is_block,n_segment,points_period,xcoord,so3d,r3dr,r3d,iskV,perror3dx)
...
%             dbfolders(k).name,', j0=',int2str(j0), ', 2D rms error=', num2str(std1)),'FontSize',18)
    ...
    elseif modex==2
        ...
        grid()
        ylabel('Position(mm)'), xlim([1 359]),...
            legend('original SI','residual SI'); 
%             title(strcat('original and residual positions of - (j0)',...
%             dbfolders(k).name,', j0=',int2str(j0), ', 2D rms error=', num2str(std1)),'FontSize',18)
        ...
        grid()
        ylabel('Position(mm)'), xlim([1 359]),...
            legend('original LR','residual LR');         
        figure(n_segment), subplot(4,1,3)
        plot(xcoord(i0:points_period),so3d(i0:points_period,3),'r',... 
        ...
        grid()
        ylabel('Position(mm)'),xlim([1 359]),...
            legend('original AP','residual AP'); 
        if is_block==0
            figure(n_segment), subplot(4,1,4)
            plot(xcoord(i0:points_period),iskV(i0:points_period),'m',...
                xcoord(i0:points_period),perror3dx(i0:points_period),'k','LineWidth',1.5)
            set(gca,'FontSize',16)
            grid()
            ylabel('Error(mm)'), xlabel('angles in degree'),...
                xlim([1 359]),legend('kV on','3D displacement');   
        else
            figure(n_segment), subplot(4,1,4)
    ...
    elseif modex==3
        ...
        grid()
        ylabel('Position(mm)'), xlim([1 359]),...
            legend('original SI','residual SI', 'checkpoint SI');
...
%             dbfolders(k).name,', j0=',int2str(j0), ', 2D rms error=', num2str(std1)),'FontSize',18)
        ...
        grid()
        ylabel('Position(mm)'), xlim([1 359]),...
            legend('original LR','residual LR', 'checkpoint LR');
